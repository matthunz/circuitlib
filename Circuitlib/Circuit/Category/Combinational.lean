...
def Hom.id [Preorder V] : CombinationalCircuitCategory.Hom V X X := ⟨fun x => x, monotone_id⟩
...
lemma whisker
    ...
    tensorHom (𝟙 X) Hom.id = 𝟙 (CombinationalCircuitCategory.of V G (X.obj + Y.obj)) := by
